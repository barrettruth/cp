...
#define FORLL(a, b, c) for (i64 a = (b); a < (c); ++a)

void solve() {
  int n, k;
  cin >> n >> k;

  vec<pair<i64, i64>> recs;
  FORI(i, 0, n) {
    cin >> recs[i].first >> recs[i].second;
  }

  vec<vec<i64>> dp(n + 1, vec<i64>(k + 1, MAX<i64>));
  FORI(i, 0, n + 1) {
    dp[i][0] = 0;
  }
  ...
  FORLL(i, 1, n + 1) {
    FORLL(j, 1, k + 1) {
      int x = recs[i - 1].first, y = recs[i - 1].second;
      int cost = 0;
      FORLL(take, 0, min(j, recs[i - 1].first + recs[i - 1].second) + 1) {
        if (dp[i - 1][j - take] != MAX<i64>)
