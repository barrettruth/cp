...
void solve() {
  ...
  hashtable<std::pair<int, int>, int> edges;
...
problem: didn't even contest this idea
  ...
  queue<int> q{{1}};
  vec<std::pair<int, int>> cuts;
  vec<int> leaves, par(n + 1, -1);
  par[1] = 1;
  // started 10:17
  ...
  while (!q.empty()) {
    ...
    if (kids(par[u]) == 2) {
      ...
      q.emplace(par[par[u]]);
      if (par[u] != par[par[u]])
        cuts.emplace_back(par[u], par[par[u]]);
    } else {
      if (seen[par[par[u]]]) {
        break;
      }
      ...
      if (par[par[u]] != par[par[par[u]]])
        cuts.emplace_back(par[par[u]], par[par[par[u]]]);
    }
  }
  ...
  for (int i = 0; i < sz<int>(cuts); ++i) {
    pr("{} ", edges[{cuts[i].first, cuts[i].second}]);
  }
  prln();
}
