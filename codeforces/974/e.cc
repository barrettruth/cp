...
void solve() {
  ...
  vec<vec<std::pair<int, i64>>> graph(n + 1);
  for (int i = 0; i < m; ++i) {
    int u, v;
    i64 w;
    cin >> u >> v >> w;
  ...
  vec<i64> marian(n + 1, MAX<i64>), robin(n + 1, MAX<i64>);

  auto djikstra = [&](int start, vec<i64>& delta) {
    priority_queue<tuple<i64, int, bool>, vec<tuple<i64, int, bool>>, greater<>>
        pq;
    delta[start] = 0;
