...
[[nodiscard]] constexpr u64 inclusive_sum(u64 l, u64 r) {
  return (r - l + 1) * (l + r) / 2;
}

[[nodiscard]] f128 quad(f128 a, f128 b, f128 c) {
...
void solve() {
  ...
  i64 half = inclusive_sum(k, k + n - 1) / 2;

  f128 i = quad(0.5, k + 0.5, -half);

  u64 a = max(1LL, static_cast<i64>(ceil(i)));
  u64 b = min(n, a + 1);

  u64 x = inclusive_sum(k, k + a - 1);
  u64 y = inclusive_sum(k + a, k + n - 1);
  u64 diff1 = x < y ? y - x : x - y;

  x = inclusive_sum(k, k + b - 1);
  y = inclusive_sum(k + b, k + n - 1);
  u64 diff2 = x < y ? y - x : x - y;
