...
void solve() {
  int n;
  cin >> n;
  vec<std::pair<i64, int>> a(n);
  for (int i = 0; i < n; ++i) {
    cin >> a[i].first;
    a[i].second = i;
  }
  sort(all(a));
  vec<i64> prefix(n + 1, 0), postfix(n + 2, 0);
  for (int i = 0; i < n; ++i) {
    prefix[i + 1] = prefix[i] + a[i].first;
  }
  for (int i = n - 1; i >= 0; --i) {
    postfix[i + 1] = postfix[i + 2] + a[i].first;
  }

  vec<i64> ans(n);
  for (int i = 0; i < n; ++i) {
    i64 x = n + a[i].first * (i + 1) - prefix[i + 1];
    x += postfix[i + 2] - a[i].first * (n - i - 1);
    ans[a[i].second] = x;
  }
